  ...
  return (
    <div className="flex flex-col items-center gap-10 ">
      ...
      <div>
        ...
        <br />
        음악을 추가하고 재생하는 기본적인 앱을 만든 후 좀 더 깊이 있게 만들고 싶어져 비주얼라이저와
        이퀄라이저 기능을 추가하였다. 
        <br />
        구글 검색으로 디자인을 참고했고 기능 구현은 직접 하였다. 가장
        익숙하고 많이 사용되는 React를 사용해 UI를 구현하였고 CSS Modules를 사용하여 순수 CSS의 감을
        잃지 않고자 하였다. 
        <br />개발 과정에서 Web Audio API의 메커니즘과 canvas API의 기본적인 기능, requestAnimationFrame() 사용법을 숙지할 수 있었다. 
        </p>
      </div>
      ...
      <h3>
        ...
        연결되는 문제.</h3> Web Audio API에서 하나의 {"<"}audio&gt;(HTMLAudioElement)는 단 하나의 오디오
        소스 노드(MediaElementAudioSourceNode)에 연결될 수 있다. 이를 해결하려면 오디오
        컨텍스트(AudioContext)와 오디오 소스 노드(MediaElementAudioSourceNode)의 여러번 연결되지
        않게 관리하거나, 새 연결을 생성할 때 기존 연결을 해제해야 한다.
        정리 -> 음악이 변경될 때마다 useEffect가 실행되어 오디오 소스 노드(MediaElementAudioSourceNode)가 생성 되었던 것을 useEffect 내에 조건문을 사용해 처음 한번만 생성하도록 수정
      <br />
      <br />
